...
/* =========================================================
   Save / Load — serializes full game state to a custom XML
   document with a .lawn file extension, and can restore it.
   ========================================================= */

var SAVE_FILE_VERSION = "1";
...
function serializeFieldGrid(field){
    // Only unlocked/generated fields have a populated grid.
    if(!field.field || field.field.length === 0){
        return "";
    }
    return field.field.map(function(column){
        return column.join(",");
    }).join(";");
}
...
function serializeField(field, index){
    var xml = "";
    xml += "    <field index=\"" + index + "\" name=\"" + xmlEscape(field.name) + "\">\n";
    xml += "      <growthAmount>" + field.growthAmount + "</growthAmount>\n";
    xml += "      <machineX>" + field.machineX + "</machineX>\n";
    xml += "      <machineY>" + field.machineY + "</machineY>\n";
    xml += "      <machineWidth>" + field.machineWidth + "</machineWidth>\n";
    xml += "      <machineHeight>" + field.machineHeight + "</machineHeight>\n";
    xml += "      <machineSpeed>" + field.machineSpeed + "</machineSpeed>\n";
    xml += "      <goingUp>" + (field.goingUp ? "1" : "0") + "</goingUp>\n";
    xml += "      <totalMowed>" + field.totalMowed + "</totalMowed>\n";
    xml += "      <tileSize>" + field.tileSize + "</tileSize>\n";
    xml += "      <tickRate>" + field.tickRate + "</tickRate>\n";
    xml += "      <superTicks>" + field.superTicks + "</superTicks>\n";
    xml += "      <superExtra>" + field.superExtra + "</superExtra>\n";
    xml += "      <upgrades>\n";
    for(var i = 0; i < field.upgrades.length; i++){
        var up = field.upgrades[i];
        xml += "        <upgrade name=\"" + xmlEscape(up.name) + "\" price=\"" + up.price + "\"/>\n";
    }
    xml += "      </upgrades>\n";
    var grid = serializeFieldGrid(field);
    if(grid.length > 0){
        xml += "      <grid>" + grid + "</grid>\n";
    }
    xml += "    </field>\n";
...
function buildSaveXml(){
    ...
    xml += "<lawnmowersave version=\"" + SAVE_FILE_VERSION + "\">\n";
    xml += "  <savedAt>" + xmlEscape(new Date().toISOString()) + "</savedAt>\n";
    xml += "  <money>" + money + "</money>\n";
    xml += "  <totalMoney>" + totalMoney + "</totalMoney>\n";
    xml += "  <mulch>" + mulch + "</mulch>\n";
    xml += "  <currentPosition>" + currentPosition + "</currentPosition>\n";
    xml += "  <unlockedFields>" + unlockedFields + "</unlockedFields>\n";
    xml += "  <fields>\n";
    for(var i = 0; i < fields.length; i++){
        xml += serializeField(fields[i], i);
    }
    xml += "  </fields>\n";
    xml += "</lawnmowersave>\n";
    return xml;
...
function saveGame(){
    var xml = buildSaveXml();
    var blob = new Blob([xml], {type: "application/xml"});
    var url = URL.createObjectURL(blob);
    var link = document.createElement("a");
    link.href = url;
    var stamp = new Date().toISOString().replace(/[:.]/g, "-");
    link.download = "lawnmower-save-" + stamp + ".lawn";
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    URL.revokeObjectURL(url);
    setSaveLoadStatus("Game saved.");
}

function setSaveLoadStatus(message){
    var el = document.getElementById("saveLoadStatus");
    if(el){
        el.innerHTML = message;
    }
}

function requireText(node, tagName){
    var child = node.querySelector(tagName);
    if(!child){
        throw new Error("Missing <" + tagName + "> in save file");
    }
    return child.textContent;
}

function parseSaveXml(xmlText){
    ...
    var root = doc.querySelector("lawnmowersave");
    if(!root){
        throw new Error("This is not a valid lawnmower save file");
    }

    var state = {
        money: parseFloat(requireText(root, "money")),
        totalMoney: parseFloat(requireText(root, "totalMoney")),
        mulch: parseFloat(requireText(root, "mulch")),
        currentPosition: parseInt(requireText(root, "currentPosition"), 10),
        unlockedFields: parseInt(requireText(root, "unlockedFields"), 10),
        fields: []
    };

    var fieldNodes = root.querySelectorAll("fields > field");
    fieldNodes.forEach(function(node){
        var index = parseInt(node.getAttribute("index"), 10);
        var fieldState = {
            index: index,
            growthAmount: parseInt(requireText(node, "growthAmount"), 10),
            machineX: parseInt(requireText(node, "machineX"), 10),
            machineY: parseInt(requireText(node, "machineY"), 10),
            machineWidth: parseInt(requireText(node, "machineWidth"), 10),
            machineHeight: parseInt(requireText(node, "machineHeight"), 10),
            machineSpeed: parseInt(requireText(node, "machineSpeed"), 10),
            goingUp: requireText(node, "goingUp") === "1",
            totalMowed: parseInt(requireText(node, "totalMowed"), 10),
            tileSize: parseInt(requireText(node, "tileSize"), 10),
            tickRate: parseInt(requireText(node, "tickRate"), 10),
            superTicks: parseInt(requireText(node, "superTicks"), 10),
            superExtra: parseFloat(requireText(node, "superExtra")),
            upgradePrices: {},
            grid: null
        };

        var upgradeNodes = node.querySelectorAll("upgrades > upgrade");
        upgradeNodes.forEach(function(uNode){
            var name = uNode.getAttribute("name");
            var price = parseFloat(uNode.getAttribute("price"));
            fieldState.upgradePrices[name] = price;
        });

        var gridNode = node.querySelector("grid");
        if(gridNode && gridNode.textContent.length > 0){
            fieldState.grid = gridNode.textContent.split(";").map(function(column){
...
function applySaveState(state){
    // Halt any in-flight tick loops (they check this flag before
    // rescheduling themselves via setTimeout).
    currentlyPrestiging = true;
    ...
    state.fields.forEach(function(fieldState){
        var field = fields[fieldState.index];
        if(!field){
            return;
        }
        field.growthAmount = fieldState.growthAmount;
        field.machineX = fieldState.machineX;
        field.machineY = fieldState.machineY;
        field.machineWidth = fieldState.machineWidth;
        field.machineHeight = fieldState.machineHeight;
        field.machineSpeed = fieldState.machineSpeed;
        field.goingUp = fieldState.goingUp;
        field.totalMowed = fieldState.totalMowed;
        field.tileSize = fieldState.tileSize;
        field.tickRate = fieldState.tickRate;
        field.superTicks = fieldState.superTicks;
        field.superExtra = fieldState.superExtra;

        Object.keys(fieldState.upgradePrices).forEach(function(name){
            var upgrade = getUpgrade(field, name);
            if(upgrade){
                upgrade.price = fieldState.upgradePrices[name];
            }
        });

        if(fieldState.grid){
            field.field = fieldState.grid;
            field.lastTick = +new Date();
        }
    });

    calculateGrowthBonus();

    // Resume a tick loop for every unlocked field (fields keep growing
    // in the background even while you're viewing a different field).
    currentlyPrestiging = false;
    for(var i = 0; i < unlockedFields; i++){
        var f = fields[i];
...
function loadGame(event){
    ...
    reader.onload = function(e){
        try{
            var state = parseSaveXml(e.target.result);
            applySaveState(state);
            setSaveLoadStatus("Loaded " + file.name);
        }catch(err){
            setSaveLoadStatus("Failed to load save: " + err.message);
        }finally{
            input.value = "";
        }
    };
    reader.onerror = function(){
        setSaveLoadStatus("Could not read that file.");
        input.value = "";
    };
